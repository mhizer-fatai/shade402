...
- **Provider allowlist:** the contract does not yet restrict `payInvoice` recipients to a provider allowlist — a known gap for the next iteration, which would also add an owner `withdraw` circuit.
